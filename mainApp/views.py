...
from .serializers import MyTokenObtainPairSerializer, CustomUserSerializer,mainDirectorySerializer,\
   ...
   PopularExpertSrializer,PopularSerializer
...
class PopularList(APIView):
    permission_classes = (permissions.AllowAny,)
    def get(self, request):
        popular = detailDirectory.objects.all()
        ...
        return Response(serializer.data)
    # def get(self, request):
    #     Product.objects.filter(Service__detailDirectory__detail_directory_name= Service__detailDirectory__detail_directory_name)
...
#     permission_classes = (permissions.AllowAny,)
#     def get(self, request):
#         queryset = Product.objects.annotate(review_count=Count('product',distinct=True)).order_by('-review_count')
#
#         serializer = ExpertProductSerializer(queryset, many=True)
#         return Response(serializer.data)
#     def post(self, request):  # 새 글 작성시
