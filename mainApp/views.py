...
from .serializers import MyTokenObtainPairSerializer, CustomUserSerializer,mainDirectorySerializer,\
    ...
    ContentSerializer,VideoReviewSerializer,ServiceSerializer,ExpertSerializer,SearchSerializer,ProductSerializer,ProductReviewSerializer,\
...
class SearchViewSet(viewsets.ModelViewSet):
    ...
    queryset = Product.objects.annotate(review_count=Count('product', distinct=True),
                                        ...
                                        category=F('Service__detailDirectory__subDirectory__sub_directory_name'))
...
class PopularExpertList(APIView):
    permission_classes = (permissions.AllowAny,)
    def get(self, request):
        queryset = Expert.objects.annotate(price_sum=Sum('expert__price'),detail_name=F('expert__Service__detailDirectory__detail_directory_name')).order_by('-price_sum')
