...
class PopularSerializer(serializers.ModelSerializer):
    # sub_name = serializers.CharField(read_only=True)
    sub_name = serializers.CharField(source='subDirectory.sub_directory_name')
    class Meta:
        model = detailDirectory
        fields = [ 'detail_directory_id','sub_name','detail_directory_name','linkUrl']
...
class PopularExpertSrializer(serializers.ModelSerializer):
    # expert_image = serializers.CharField(source='user.profile_image')
    profile_image = serializers.URLField(source='user.profile_image')
    ...
    class Meta:
        model = Expert
        fields = ['expert_id', 'company_name', 'is_connect', 'expert_description','price_sum','profile_image','expert','service']
        # depth=2
# #상세분야
...
class SearchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['product_id','product_name','heart_count','rating_avg','review_count','created_at','Service','expert']
        # fields= ( '__all__')
        # depth=3
# #서브 디렉토리 아이콘
